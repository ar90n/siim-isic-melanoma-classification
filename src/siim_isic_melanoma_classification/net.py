...
class EfficientNetB2MLP(LightningModelBase):
    name: str = "en_b2_mlp"

    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)
        self._backbone = timm.create_model(
            "efficientnet_b2a", num_classes=512, pretrained=True
        )
...
class EfficientNetB4MLP(LightningModelBase):
    name: str = "en_b4_mlp"

    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)
        self._backbone = timm.create_model(
            "tf_efficientnet_b4_ns", num_classes=512, pretrained=True
        )
        self._meta = nn.Sequential(
            nn.Linear(9, 512),
            nn.BatchNorm1d(512),
...
class EfficientNetB1MLP(LightningModelBase):
    name: str = "en_b1_mlp"

    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)
        self._backbone = timm.create_model(
            "efficientnet_b1", num_classes=512, pretrained=True
        )
        self._meta = nn.Sequential(
            nn.Linear(9, 512),
            nn.BatchNorm1d(512),
